  ...
  const form = formidable({
    ...
    filter: ({ name, originalFilename, mimetype }) => {
      ...
      if (!valid) {
        form.emit(
          "error" as any,
          new BadRequestError("File type or filename is not valid") as any
        );
      }
...
export const parseCloudinaryUrl = (url: string): IMedia | null => {
  ...
  try {
    ...
    const cloudNameIndex = pathParts.findIndex(
      (p) => p === process.env.CLOUDINARY_CLOUD_NAME
    );
    ...
    throw new BadRequestError(
      `Không thể parse URL Cloudinary: ${url} - ${
...
export const signedUrls = ({
  ...
  public_data_signed: SignedDto;
}): Promise<IMedia[]> => {
  if (
    !public_data_signed ||
    !Array.isArray(public_data_signed) ||
    public_data_signed.length === 0
  ) {
    throw new BadRequestError(
      "public_data_signed thì phải là một mảng không rỗng"
    );
  }
  ...
  return public_data_signed.map((x) => {
    const url = cloudinary.url(x.public_id, {
      ...
      expires_at:
        ...
        Number(process.env.CLOUDINARY_ACCESS_EXPIRES), // hết hạn sau 15 phút
    ...
    return {
      url,
      resource_type:
        (x.resource_type as unknown as EMediaType) || EMediaType.Image,
