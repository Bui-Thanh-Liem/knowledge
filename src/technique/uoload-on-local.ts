...
import {
  UPLOAD_IMAGE_FOLDER_PATH,
  UPLOAD_VIDEO_FOLDER_PATH,
} from "~/shared/constants/path-static.constant";
...
export function uploadImages(req: Request): Promise<string[]> {
  ...
  const form = formidable({
    ...
    filter: ({ name, originalFilename, mimetype }) => {
      const valid =
        name === "images" && Boolean(mimetype && mimetype?.includes("image/"));
      ...
      (async () => {
        try {
          const compressedFiles = await Promise.all(
            imageUploaded.map((img) => compressionFile(img))
          );
          const imgMap = compressedFiles.map(
            (img) => `${envs.SERVER_DOMAIN}/uploads/${img}`
          );
          res(imgMap);
        } catch (error) {
          rej(error);
...
export function uploadVideos(req: Request): Promise<string[]> {
  ...
  const form = formidable({
    ...
    filter: ({ name, originalFilename, mimetype }) => {
      const valid =
        name === "videos" && Boolean(mimetype && mimetype?.includes("video/"));
